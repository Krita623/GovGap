...
PROBLEM_ERROR_MARKERS = (
    "Anvil was unavailable",
    "RemoteDisconnected",
    "ConnectionAborted",
    "Read timed out",
    "timed out",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Rerun only samples whose existing trace output has fallback or transient execution errors."
    )
    ...
    parser.add_argument(
        "--require-anvil",
        action="store_true",
        help="Stop if a rerun still falls back to static_decode or another non-simulated trace source.",
    )
    ...
    sample_paths = _find_problem_samples(args.samples_dir, args.traces_dir)
    rows: list[dict[str, Any]] = []
    start = perf_counter()

    if not sample_paths:
        _write_outputs(args.summary_dir, rows, 0.0)
        print("No problem trace outputs found.", flush=True)
        return

    print(f"Rerunning {len(sample_paths)} problem trace output(s).", flush=True)
    ...
    for index, sample_path in enumerate(sample_paths, start=1):
        ...
        try:
            ...
            if args.require_anvil and trace_result.trace_source != "simulated_trace":
                row["error"] = row["error"] or "required simulated_trace but got fallback trace source"
...
def _find_problem_samples(samples_dir: Path, traces_dir: Path) -> list[Path]:
    sample_by_id = {path.stem: path for path in samples_dir.glob("*.json")}
    problem_samples: list[Path] = []

    for trace_path in sorted(traces_dir.glob("*-trace.json")):
        benchmark_id = trace_path.name[: -len("-trace.json")]
        sample_path = sample_by_id.get(benchmark_id)
        if not sample_path:
            continue
        if _is_problem_trace(trace_path):
            problem_samples.append(sample_path)

    return problem_samples


def _is_problem_trace(trace_path: Path) -> bool:
    try:
    ...
    if trace_source == "static_decode":
        return True

    error = data.get("error")
    if isinstance(error, str) and error:
        return any(marker.lower() in error.lower() for marker in PROBLEM_ERROR_MARKERS)

    return False


def _summarize(rows: list[dict[str, Any]], elapsed_seconds: float) -> dict[str, Any]:
    ...
    return {
        ...
        "trace_without_calls": sum(1 for row in rows if int(row["call_count"]) == 0),
...
def _write_outputs(summary_dir: Path, rows: list[dict[str, Any]], elapsed_seconds: float) -> None:
    summary = _summarize(rows, elapsed_seconds)
    (summary_dir / "rerun-problem-traces-summary.json").write_text(
        json.dumps({"summary": summary, "rows": rows}, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )

    with (summary_dir / "rerun-problem-traces-summary.csv").open("w", encoding="utf-8", newline="") as file:
